...
export class CreateSmsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.initForm();
    if(this.fromAdv) {
      const users = this.storageService.get('usersToSms');
      if(users) {
        this.freelancers = JSON.parse(users).map((freelancer: any) => {
          return {
            ...
            mobile: (freelancer?.mobile_dial_code ? freelancer?.mobile_dial_code : '') + ' ' + (freelancer?.mobile ? freelancer?.mobile : '')
          }
        });
  ...
  ngOnDestroy(): void {
    this.storageService.clear('usersToSms');
  ...
  createSms() {
    if (this.detailForm) {
      const dialogRef = this.dialog.open(ConfirmBoxComponent, {
        data: {
          type: 'warning',
          title: this.translateService.instant('sms.send-sms.title'),
          message: this.translateService.instant('sms.send-sms.message', { count: this.selection.selected.length }),
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          if (this.detailForm) {
            ...
            this.smsService.create(this.selection.selected, obj.campaign, obj.purpose, obj.message).subscribe((res: any) => {
              this.reset();
              let msg = this.translateService.instant('notification.post.mails.success');
              if(res && res.body) {
                msg = `Total Recipients : ${res.body?.total_recipients} <br/> SMS Sent : ${res.body?.sms_sent} <br/> SMS Not Sent : ${res.body?.sms_not_sent}`;
              }
